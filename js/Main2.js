window.onload = function() {
			...
			setTimeout(() => {
				...
				if (width<=height) {
					...
					document.querySelector("meta[name=viewport]").setAttribute(
						'content', 
						`initial-scale=${scaleCanva}, user-scalable=no`);
	...
	setTimeout(() => {
		...
		// game.scale.compatibilityorientationFallback = 'viewport';
		// game.scale.compatibility.forceMinimumDocumentHeight = true;
		// game.scale.compatibility.canExpandParent = false;
		// game.scale.maxWidth = window.innerHeight;
		// game.scale.margin = 20;
	}, 1);
